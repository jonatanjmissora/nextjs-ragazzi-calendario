...
export default function MenuLinks() {
  ...
  return (
    <nav className='mt-auto flex flex-col gap-3'>
      ...
      <div className={`${pathname.includes("/admin") ? "block" : "hidden"} flex flex-col gap-3`}>
        <Link className={`w-full text-center font-bold tracking-wider px-2 py-1 rounded-xl ${pathname === "/admin/sectores" && "bg-slate-400 text-slate-900"}`} href={"/admin/sectores?type=actuales"}>sectores</Link>
        <Link className={`w-full text-center font-bold tracking-wider px-2 py-1 rounded-xl ${pathname === "/admin/weblinks" && "bg-slate-400 text-slate-900"}`} href={"/admin/weblinks"}>weblinks</Link>
      </div>
    </nav>
  )
}
